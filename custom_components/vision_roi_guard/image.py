...
async def async_setup_entry(
    ...
    async_add_entities(
        [
            LastAnalyzedImageEntity(hass, runtime_data.coordinator, entry),
            RoiEditorImageEntity(hass, runtime_data.coordinator, entry),
        ]
    )
...
class RoiEditorImageEntity(VisionRoiGuardEntity, ImageEntity):
    _attr_translation_key = "roi_editor_image"
    _attr_content_type = "image/png"

    def __init__(self, hass: HomeAssistant, coordinator, entry: ConfigEntry) -> None:
        VisionRoiGuardEntity.__init__(self, coordinator, entry.title)
        ImageEntity.__init__(self, hass)
        self._attr_unique_id = f"{entry.entry_id}_roi_editor_image"

    @property
    def image_last_updated(self):
        return self.coordinator.state.roi_editor_image_updated_at

    def image(self) -> bytes | None:
        image_path = self.coordinator.state.roi_editor_image_path
        if image_path is None:
            return None
        path = Path(image_path)
        if not path.is_file():
